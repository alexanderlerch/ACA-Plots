function plotSelfSimilarityMatrix ()
    ...
    hold on
    plot(vrms,tvrms, 'Color', [.1 .1 .1])
    plot(-vrms,tvrms, 'Color', [.1 .1 .1])
    hold off
    for i=2:length(tannot)-1
        line([-1.1 1],[tannot(i) tannot(i)], 'Color', [234/256 170/256 0],'LineWidth',1)
    end
    axis([ -1 1 t(1) t(end)])
...
function [tv, Dv, tvrms, vrms, t, x, tannot, annot] = getData(cAudioPath, cName)
    ...
    [vrms, tvrms] = ComputeFeature (deblank(cFeatureNames(2,:)), x, fs, [], iWindowLength, iHopLength);
    vrms = 10.^(vrms(1,:)*.05);
    vrms = vrms/max(vrms);
